...
def apply_discount_rate(simulation_results, annual_discount_rate):
    ...
    for scenario, df in simulation_results.items():
        # Calculate discounted cumulative costs and other metrics
        ...
        # Add the updated DataFrame to the results
...
def plot_simulations(discounted_results, legend_title):
    # Define scenarios and corresponding colors for plotting
    scenarios = discounted_results.keys()
    ...
    metrics = [
        ...
        ('Discounted Annual Costs', 'Cost (per $1k)', 'Discounted Annual Costs'),
        ('Discounted Cumulative Costs', 'Cost (per $1k)', 'Discounted Cumulative Costs Over Time'),
        ('Discounted CTLA Value', 'CTLA Value (per $1k)', 'Discounted CTLA Value of All Trees Over Time'),
        ('Discounted Net Value', 'Net Value (per $1k)', 'Discounted Net Value of All Trees Over Time'),
    ]
    ...
    for idx, (metric, ylabel, title) in enumerate(metrics):
        ...
        for scenario, color in zip(scenarios, colors):
            data = discounted_results[scenario].copy()

            # Scale Cumulative Costs to per $1,000
            data_copy = data.copy()
            if metric in {'Discounted Annual Costs', 'Discounted Cumulative Costs', 'Discounted CTLA Value', 'Discounted Net Value'}:
...
discounted_results = apply_discount_rate(all_results, annual_discount_rate)
plot_simulations(discounted_results, legend_title)

## ----------------------------------------------- OPTIMIZATION FUNCTION -----------------------------------------------
# Extract and print the removal rate achieving the highest value for each metric at Year 20
metrics = [
    'Total Tree Count',  # Non-discounted metric remains
    'Total Tree Basal Area',  # Non-discounted metric remains
    'Discounted Cumulative Costs',  # Updated metric
    'Discounted CTLA Value',  # Updated metric
    'Discounted Net Value',  # Updated metric
]

# Initialize dictionary to store the optimization for each metric
best_combinations = {}

# Iterate over discounted results
for metric in metrics:
    best_combination = None
    best_value = float('-inf') if metric != 'Discounted Cumulative Costs' else float('inf')

    for key, results in discounted_results.items():
        ...
        if metric == 'Discounted Cumulative Costs':  # Minimize cumulative costs
...
for metric, ((removal_rate, injection_year), value) in best_combinations.items():
    if metric == 'Discounted Cumulative Costs':
        print(f"{metric}: Removal Rate = {removal_rate}, Injection Year = {injection_year}, Lowest Value = ${value:.2f}k")
    else:
        print(f"{metric}: Removal Rate = {removal_rate}, Injection Year = {injection_year}, Highest Value = ${value:.2f}k")

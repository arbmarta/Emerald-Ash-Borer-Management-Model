...
# Function to obtain present values
...
# Function to extract and report year 20 counts with discounting
...
# Function to print future values of Years 1, 5, 10, 15, and 20 to a .csv
def report_years_future_values(simulation_results_future_value, years_to_report, output_csv):
    all_years_data = []

    for scenario, df in simulation_results_future_value.items():
        for year in years_to_report:
            year_data = df[df['Year'] == year]
            if not year_data.empty:
                # Extract relevant values
                data = {
                    "Metric": "Cumulative Costs",
                    "Scenario": scenario,
                    f"Year {year}": round(year_data['Cumulative Costs'].values[0], 2)
                }
                all_years_data.append(data)

                data = {
                    "Metric": "CTLA Value of All Trees",
                    "Scenario": scenario,
                    f"Year {year}": round(year_data['CTLA Value of All Trees'].values[0], 2)
                }
                all_years_data.append(data)

                data = {
                    "Metric": "Net Value of All Trees",
                    "Scenario": scenario,
                    f"Year {year}": round(year_data['Net Value of All Trees'].values[0], 2)
                }
                all_years_data.append(data)

                # Add more metrics as needed, e.g., tree counts or basal areas
                for metric in ['Ash Tree Count', 'Non-Ash Tree Count', 'Total Tree Count',
                               'Ash Tree Basal Area', 'Non-Ash Tree Basal Area', 'Total Tree Basal Area']:
                    data = {
                        "Metric": metric,
                        "Scenario": scenario,
                        f"Year {year}": round(year_data[metric].values[0], 2)
                    }
                    all_years_data.append(data)
...
# Specify the years to report
years_to_report = [1, 5, 10, 15, 20]

# Run the function and save results
output_csv = "Supplementary Material - Future Values Table.csv"

# Function to print present values of Years 1, 5, 10, 15, and 20 to a .csv
...
# Run the function and save results
output_csv = "Supplementary Material - Present Values Table.csv"

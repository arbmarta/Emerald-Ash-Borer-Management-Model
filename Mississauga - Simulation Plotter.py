...
def plot_simulations_colour(simulation_results_future_value):
...
def plot_simulations_black_and_white(simulation_results_future_value):
    scenarios = simulation_results_future_value.keys()
...
def plot_simulations_colour_present_value(simulation_results_present_value):
    scenarios = simulation_results_present_value.keys()
    colors = ['red', 'pink', 'blue', 'orange', 'green', 'purple', 'black']
    ...
    for idx, (metric, ylabel, title) in enumerate(metrics, start=1):
        ax = plt.subplot(3, 2, idx)  # Get axis handle
        for scenario, color in zip(scenarios, colors):
            data = simulation_results_present_value[scenario].copy()

            # Scale specific metrics to per $1,000
    ...
    # Adjust layout for space around the legend
    plt.tight_layout(rect=[0, 0, 1, 1])  # Reserve space on the right for the legend
    plt.savefig("Figure 2 - Colour.jpeg", format='jpeg')
    plt.show()

def plot_simulations_black_and_white_present_value(simulation_results_present_value):
    scenarios = simulation_results_present_value.keys()
    colors = ['black'] * len(scenarios)
    lines = ['solid', 'dotted', 'dashed', 'dashdot', (0, (3, 5, 1, 5, 1, 5)), (0, (3, 10, 1, 10)), (0, (5, 5))]
    ...
    # Adjust layout for space around the legend
    plt.tight_layout(rect=[0, 0, 1, 1])  # Reserve space on the right for the legend
    plt.savefig("Figure 2 - B&W.jpeg", format='jpeg')
    plt.show()

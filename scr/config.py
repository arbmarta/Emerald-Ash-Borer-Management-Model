...
# Figure options
plot_style = 'colour' # Select either 'colour' or 'monochrome'
output_dir_fig = '../outputs/figures'
output_dir_csv = '../outputs/tables'
dpi = 600

# Simulation Parameters
removal_rate = 400 # Sets a number of ash trees to remove each year
planting_rate = 400 # Sets a number of new trees to plant each year
removal_year = 5 # Select year in which ash trees begin to be removed
injection_years = 5 # Number of years to inject ash
planting_year = 1 # Year where tree planting starts
pruning_cycle_years = 7 # Each tree is pruned roughly once every this many years

# Reporting
years_to_report = [1, 5, 10, 15, 20]
output_csv_future_values = "future_values.csv"
output_csv_present_values = "present_values.csv"
...
# Mortality rate data for new plantings, by age since planting (years 1-4).
# Any age not listed here (i.e. age >= 5) falls back to background_mortality_rate below.
mortality_rates_by_age_overrides = {
    1: 0.085, # Year 1 mortality rate for new plantings
    2: 0.117, # Year 2 mortality rate for new plantings
    3: 0.042, # Year 3 mortality rate for new plantings
    4: 0.061, # Year 4 mortality rate for new plantings
}

# Define mortality rates by age with default for age >= 5
mortality_rates_by_age = {
    age: mortality_rates_by_age_overrides.get(age, background_mortality_rate)
